...
/** Client that forwards the verified OAuth token so RLS runs as the signed-in user. */
...
export async function resolveOrgId(ctx: ToolContext): Promise<string> {
  ...
  if (!data) throw new Error("You are not a member of the onboarding organization.");
